...
- **Node.js 的 Event Loop** 与浏览器不同，有 6 个阶段（见下方详解）
...
1. **每个阶段之间**都会清空微任务队列（Promise.then、queueMicrotask）
2. **process.nextTick** 优先级高于 Promise 微任务，在每个阶段切换时最先执行
3. **setTimeout vs setImmediate**：
   - 在主模块中，顺序不确定（取决于系统调度延迟）
   - 在 I/O 回调中，`setImmediate` 总是先于 `setTimeout`
4. **process.nextTick 递归风险**：与浏览器微任务递归类似，会阻塞事件循环进入下一阶段

### 微任务优先级（Node.js）

```
process.nextTick 队列（最高）
    ↓
Promise 微任务队列
    ↓
当前阶段的下一个回调 / 进入下一阶段
...
| nextTick | 无 | 有，优先级最高 |
| setImmediate | 非标准（仅 IE） | check 阶段执行 |
| I/O 回调 | 宏任务队列 | poll 阶段 |
